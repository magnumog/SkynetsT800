...
Poly1 = [-0.8,0.6;-0.2,-0.2;0.4,-0.7;0.5,0.2;0.2,0.3;-0.296057,0.596997];
Poly2 = [-0.6,-0.4;0.0,0.1;0.9,-0.6;0.4,0.1;0.102922,0.598169;-0.3,0.4];
...
sampleList = [];
%makes a random matrix (2000x2) for the points
randomSamplesMatrix = rand(numSamples,2);
% disp(size(randomSamplesMatrix(:,1)));
% The sample list reason for negative pi is because the intervall is from
% -pi to pi not -pi to 2pi
sampleList = -pi +2*pi*randomSamplesMatrix;
...
% connectRadius = (4.0*pi)/(numSamples/2);
connectRadius = (1.6*pi)/(numSamples/100);
...
filename = '2-linkMoviebPRMExtra.avi';
delete(filename);
movieObj = avifile('2-linkMoviebPRMExtra.avi');
...
for row = 1:size(cSpacePath,1)
    ...
    fill( drawLink1(:,1), drawLink1(:,2), '.red' )
    hold on
    fill( drawLink2(:,1), drawLink2(:,2), '.green' )

    for i = 1:length(ObstacleList)
        obs = ObstacleList{i};
        fill(obs(:,1),obs(:,2),'.yellow')
    end
